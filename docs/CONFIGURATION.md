...
| `S3_SCREENSHOT_BUCKET_NAME` | (Legacy) S3 bucket for storing interaction screenshots |
...
You can use the provided script to automatically fetch AWS outputs after a SAM deployment:
```bash
cd backend
./get-env-vars.sh <stack-name> --update-env
```
This will update your root `.env` file with the correct API Gateway URLs, Bucket names, and Cognito IDs.
