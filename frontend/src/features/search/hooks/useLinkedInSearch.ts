...
export function useLinkedInSearch({ fetchConnections }: UseLinkedInSearchOptions) {
  ...
  const handleLinkedInSearch = useCallback(async (filters: { company: string; job: string; location: string; userId: string }) => {
    ...
    try {
      ...
      toast({ title: "Search Failed", description: "Failed to search LinkedIn. Please try again.", variant: "destructive" });
    } finally {
