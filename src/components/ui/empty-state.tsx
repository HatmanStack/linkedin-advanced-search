...
const EmptyState: React.FC<EmptyStateProps> = ({
  ...
  return (
    <div className={cn(
      ...
      {icon && (
        <div className={cn('text-slate-400 opacity-50 mb-4', classes.icon)}>
          {icon}
        </div>
      )}
      
      <h3 className={cn('font-medium text-slate-300 mb-2', classes.title)}>
        {title}
      </h3>
      
      {description && (
        <p className={cn('text-slate-400 max-w-md', classes.description)}>
          {description}
        </p>
      )}
    ...
    switch (type) {
      case 'all':
        return {
          ...
          description: 'Start building your network by searching for new connections or importing your existing LinkedIn contacts.',
          action: onRefresh ? {
            label: 'Refresh',
            onClick: onRefresh,
      ...
      case 'filtered':
        return {
          ...
          description: 'No connections match your current filter. Try selecting a different status or clearing your filters.',
      ...
      case 'new':
        return {
          ...
          description: 'You\'re all caught up! Use the search feature above to discover new potential connections.',
          action: onRefresh ? {
            label: 'Refresh',
            onClick: onRefresh,
            variant: 'outline' as const
